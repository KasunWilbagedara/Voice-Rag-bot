...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md animate-fade-in p-4">
      <div className="w-full max-w-xl glass-panel bg-[#08080a] border border-white/15 rounded-3xl p-6 md:p-7 flex flex-col gap-5 relative shadow-2xl text-slate-100 max-h-[90vh] overflow-y-auto custom-scrollbar">
        {/* Header */}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              ...
              <button
                ...
                className={`py-2.5 px-3 rounded-2xl text-xs font-bold transition-all flex flex-col items-center gap-0.5 border ${provider === p.id
                    ? 'bg-gradient-to-b from-amber-400 to-amber-500 text-black border-amber-300 shadow-lg shadow-amber-500/20 font-extrabold'
                    : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/15 hover:text-white'
                  }`}
              >
                <span className="text-sm">{p.icon}</span>
                <span>{p.label}</span>
                <span className={`text-[9px] ${provider === p.id ? 'text-black/80 font-bold' : 'text-slate-500'}`}>
                  {p.sub}
                </span>
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {(provider === 'gemini'
              ...
              <button
                ...
                className={`py-2 px-3 rounded-xl text-xs font-semibold border text-center transition-all ${customModel === m.id
                    ? 'bg-gradient-to-b from-amber-400 to-amber-500 text-black border-amber-300 font-extrabold shadow-md shadow-amber-500/20'
                    : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/15 hover:text-white'
                  }`}
              >
                {m.label}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {[
              ...
              <button
                ...
                className={`py-2 px-2.5 rounded-xl text-[11px] font-semibold border text-center transition-all ${voice === v.id
                    ? 'bg-gradient-to-b from-amber-400 to-amber-500 text-black border-amber-300 font-extrabold shadow-md shadow-amber-500/20'
                    : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/15 hover:text-white'
                  }`}
              >
        ...
        <button
          onClick={handleSave}
          className="w-full py-3 rounded-2xl realistic-button-amber text-black font-extrabold text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-amber-500/25 mt-2 active:scale-98"
        >
          {isSaved ? (
            <>
              <Check className="w-4 h-4 text-black stroke-[3]" /> Saved Configuration!
            </>
          ) : (
            <>
